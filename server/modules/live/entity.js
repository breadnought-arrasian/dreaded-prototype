...
class Entity extends EventEmitter {
    ...
    define(defs, emitEvent = true) {
        ...
        if (set.GROW_DAMAGE != null) {
            this.growdmg = set.GROW_DAMAGE;
        }
    ...
    get size() {
        if (this.bond == null)
            return (this.coreSize || this.SIZE) * (1 + Math.min(this.skill.level, 45) / 45);
    ...
    camera(tur = false) {
        return {
            ...
            twiggle: this.facingType === "autospin" || this.facingType === "spinSlowly" || this.facingType === "spinEvenSlower" || (this.facingType === "locksFacing" && this.control.alt),
    ...
    face() {
        ...
        switch (this.facingType) {
            ...
            case "turnWithSpeed":
                ...
                break;
            case "turnSlowlyWithSpeed":
                this.facing += ((this.velocity.length / 90) * Math.PI) / roomSpeed / 2;
                break;
            case "turnSlowlyWithSpeed2":
                this.facing += ((this.velocity.length / 90) * Math.PI) / roomSpeed / 5;
                break;
            case "spin":
        ...
        if (this.growdmg) { this.damage += this.growdmg }
        // Loop
        const TAU = 2 * Math.PI;
    ...
    contemplationOfMortality() {
        ...
        if (this.isDead()) {
            ...
            if (this.nextForm) { // NEXT_FORM‚ÌÝ’è‚ª‚ ‚Á‚½‚ç
                let position = {
                    x: this.x,
                    y: this.y,
                    facing: this.facing,
                ...
                o.define(this.nextForm); // Entity‚ðNEXT_FORM‚ÉÝ’è‚³‚ê‚½íŽÔ‚É‚·‚é
                o.team = this.team;
